...
public class WizardSimulator implements Solver<Integer> {
    ...
    private int findMinManaToWin(Boss boss, boolean hardMode) {
        ...
        queue.offer(new GameState(PLAYER_HP, PLAYER_MANA, boss.hp, 0, new HashMap<>(), true));
    ...
    private GameState applyEffects(GameState state) {
        ...
        int playerArmor = 0;
        Map<Effect, Integer> newEffects = new HashMap<>();
    ...
    private GameState castSpell(GameState state, Spell spell, int bossDamage) {
        ...
        Map<Effect, Integer> effects = new HashMap<>(state.effects);
