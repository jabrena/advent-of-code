...
| Day3 | `Day3.java` | O(L) | O(L) ✅ | Part 1: For each line, converts to digits O(n), then finds max digit sequence of length 2 using greedy approach O(n). Part 2: Same approach with length 12, still O(n) per line. Overall O(L) where L is total characters. | ✅ **OPTIMAL**: Uses greedy algorithm to find maximum digit sequence. Each line processed in O(n) time where n is line length. | ✅ **OK** - Already optimal greedy solution |
...
- For each line:
  - Converts string to digit array: O(n) where n is line length
  - Finds maximum digit sequence of length 2 using greedy approach:
    - Iterates through remaining positions to find max digit
    - For length=2: O(n) total operations
- **Overall: O(L)** ✅

**Part 2 Analysis:**
- Same structure as Part 1 but with length=12
- For each line:
  - Converts to digit array: O(n)
  - Finds maximum digit sequence of length 12:
    - Greedy approach: 12 iterations, each scans portion of array
    - Total operations per line: O(n) (each position visited at most once)
- **Overall: O(L)** ✅

**Algorithm Notes:**
- Uses greedy algorithm: at each step, selects maximum digit from remaining valid positions
- Maintains `currentPos` to ensure we can form a sequence of required length
...
- **Day3**: Efficient greedy algorithm implementation. Clean separation of concerns with `solve()` and `getMaxJoltage()` methods. Uses imperative style appropriately for this problem type. ✅ Already optimal.
