...
- **V**: Number of vertices in graph (Day11)
...
2. ✅ **Day1 Part 2**: Optimized from O(n×d) to O(n) by calculating zero crossings directly using modular arithmetic.
3. ✅ **Day2 Part 2**: Optimized from O(R×L²) to O(R×L) by using direct character comparison instead of substring creation.
4. ✅ **Day8 Part 1**: Optimized from O(n² log n) to O(n² log k) where k=1000 << n, using priority queue approach (~19% faster).
5. ✅ **Day9 Part 2**: Optimized constant factors (~7.4x faster) with pre-computed vertical edges, parallel processing, and quick bounds filtering. Big O remains O(n³) (optimal).
