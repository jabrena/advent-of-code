...
- **Day9**: Part 1 is optimal O(n²) - must examine all point pairs to find maximum area rectangle. Part 2 is optimal O(n³) - generates all point pairs O(n²) and validates each rectangle candidate against all polygon edges O(n) using point-in-polygon and edge intersection checks. This complexity is necessary for geometric polygon containment validation.
...
- `findMaxRectangleInPolygon()`:
  ...
  - For each pair, `calculateValidArea()`:
    ...
    - `isPointInPolygon()`: O(n) - checks if midpoint is on any edge O(n), then ray casting with vertical edges O(n)
    - `edgesIntersectRect()`: O(n) - checks all edges for intersection with rectangle interior
    - Total per pair: O(n)
  - Filters valid areas and finds maximum: O(n²)
- **Overall: O(n³)** ✅ (dominated by validating each rectangle candidate against all edges)

**Algorithm Notes:**
- Uses Stream API for functional programming style
- Part 1 uses straightforward approach: generate all pairs, calculate area, find maximum
- Part 2 uses geometric validation:
...
**Potential Optimizations:**
- Part 2 could potentially be optimized using spatial data structures (e.g., R-tree) to reduce edge checks, but for typical input sizes (n points, n edges), the O(n³) approach is practical and optimal
- The algorithm is already optimal - must examine all pairs and validate against all edges for geometric correctness
...
- **Day9**: Clean functional implementation with MaxRectangleArea. Uses Stream API for declarative transformations. Good separation of concerns with `Point`, `Edge`, and `PointPair` records. Part 1 finds maximum area rectangle from all point pairs. Part 2 uses geometric validation (ray casting for point-in-polygon, edge intersection checks) to find maximum valid rectangle inside polygon. All operations are pure functions except I/O boundary. Follows FP principles. ✅ Optimal solution - must examine all pairs and validate against all edges for geometric correctness.
