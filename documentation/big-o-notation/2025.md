...
- Finds max length: O(R×C) where C is max column length
- Pads lines: O(R×C)
...
- For each block:
  - `processBlockPart2()`:
    ...
    - Total per block: O(R×W + W×R) = O(R×W)
  - Total: O(B×R×W) = O(R×C)
- **Overall: O(R×C)** ✅
...
- Part 2 processes blocks vertically (column by column, right to left):
  ...
  - Processes columns in reverse order (right to left)
...
*Day6 added with MathBlockSolverV2 analysis*
