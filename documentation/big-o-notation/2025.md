...
| Day8 | [`PointCluster.java`](../../2025/src/main/java/info/jab/aoc2025/day8/PointCluster.java) | O(n² log n) | O(n² log n) ✅ | 0.393s | Part 1: Generates all point pairs O(n²), sorts by distance O(n² log n), unions top 1000 using DSU. Part 2: Same approach, unions all connections until single component. | ✅ **OPTIMAL**: Both parts generate all pairs and sort them. DSU operations are effectively O(1) per operation. Sorting is necessary to process connections in distance order. | ✅ **OK** - Optimal solution for geometric clustering problem |
...
- **O(n²)**: Day9 Part 1 ✅
- **O(n² log n)**: Day8 Part 1 ✅, Day8 Part 2 ✅
- **O(n³)**: Day9 Part 2 ✅
...
- **Moderate (0.2-0.5s):** Days 2 (0.308s), 8 (0.393s) (2 days)
- **Slow (> 0.5s):** None
...
- Day 8 is the slowest at 0.393s (geometric clustering with sorting)
...
3. ✅ **Day9 Part 2**: Optimized constant factors (~7.4x faster) with pre-computed vertical edges, parallel processing, and quick bounds filtering. Big O remains O(n³) (optimal).
...
- `getSortedConnections()`:
  ...
  - Sorts connections by distance: O(n² log n²) = O(n² log n)
...
- Gets component sizes: O(n × α(n)) ≈ O(n) - finds root for each element, then gets distinct roots
- Sorts component sizes and takes top 3: O(n log n) for sorting, but could be optimized to O(n) with selection algorithm
- **Overall: O(n² log n)** ✅ (dominated by sorting all connections)

**Part 2 Analysis:**
- Parses n points: O(n)
- `getSortedConnections()`: O(n² log n) - same as Part 1
- Creates DSU: O(n)
...
- `getSortedConnections()` generates all pairs of points (n choose 2 = n(n-1)/2 ≈ n²) and sorts them by distance squared
...
3. ✅ **Day9 Part 2**: Optimized constant factors with pre-computed vertical edges, parallel processing, and quick bounds filtering (~7.4x faster)
