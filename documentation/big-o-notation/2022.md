...
| Day8 | `Day8.java` | O(R×C×max(R,C)) | O(R×C×max(R,C)) | R×C = grid size. For each cell, checks visibility in 4 directions. | Could optimize by precomputing visibility from edges. | 🟡 MEDIUM - Can optimize with dynamic programming |
| Day9 | `Day9.java` | O(n×s) | N/A | n = movements, s = steps per movement. Part 2 not implemented. | Part 2 needs implementation. | 🔴 HIGH - Part 2 not implemented |
