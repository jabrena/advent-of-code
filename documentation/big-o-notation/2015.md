...
|-----|------|-------------|--------------|-------|--------------|-------------------|
| Day1 | `Lisp2.java` | O(n) | O(n) | Linear scan through input characters. Part 2 tracks position when floor becomes -1. | Already optimal. No improvements needed. | ✅ OK - Already optimal |
| Day2 | `Present2.java` | O(n) | O(n) | Linear scan through presents. Each present calculation is O(1). | Already optimal. No improvements needed. | ✅ OK - Already optimal |
| Day3 | `Houses3.java` | O(n) | O(n) | Linear scan tracking positions. Part 2 alternates between Santa and Robo-Santa. | Already optimal. No improvements needed. | ✅ OK - Already optimal |
| Day4 | `AdventCoinMiner.java` | O(k) | O(k) | Brute force search for MD5 hash. k = number to find (can be large). | ✅ **Refactored**: Added parallel processing using `IntStream.parallel()` for improved performance on multi-core systems. Each thread creates its own MessageDigest instance for thread-safety. | ✅ OK - Optimized with parallel processing |
| Day5 | `StringValidator.java` | O(n×L) | O(n×L) | n = strings, L = average string length. Part 2 has more complex validation. | Already optimal. String operations are necessary. | ✅ OK - Already optimal |
| Day6 | `LightCounter.java` | O(n×A) | O(n×A) | n = commands, A = average area of rectangles. Processes each rectangle cell. | ✅ **Refactored**: Improved code organization with functional approach using `IntUnaryOperator` for command operations. Reduced code duplication by extracting common `applyCommand` method. Enhanced parsing logic with better variable naming. | ✅ OK - Improved code quality and maintainability |
| Day7 | `Circuit.java` | O(G) | O(G) | G = gates. Uses topological sort to process gates in dependency order. | Already optimized with topological sort. | ✅ OK - Optimized with topological sort |
| Day8 | `StringLiteralCalculator.java` | O(n×L) | O(n×L) | n = strings, L = average string length. Processes each character for encoding/decoding. | Already optimal. Character-by-character processing is necessary. | ✅ OK - Already optimal |
| Day9 | `RouteOptimizer.java` | O(n!) | O(n!) | Generates all permutations of cities. n = number of cities. Exponential factorial complexity. | ✅ **Refactored**: Replaced permutation generation with recursive branch-and-bound approach. Fixed first city to reduce from n! to (n-1)! permutations. More memory-efficient recursive traversal instead of generating all permutations upfront. Cleaner code structure with unified `findOptimalRoute` method for both min/max. | ✅ OK - Optimized with recursive branch-and-bound, reduced memory usage |
| Day10 | `LookAndSay.java` | O(i×L) | O(i×L) | i = iterations (40/50), L = sequence length (grows exponentially). | ✅ **Refactored**: Improved code readability and maintainability. Extracted `appendRun` method for better separation of concerns. Added empty string check for edge case handling. Enhanced variable naming for clarity. | ✅ OK - Improved code quality and readability |
| Day11 | `PasswordValidator.java` | O(k) | O(k) | k = number of passwords to check. Increments and validates passwords sequentially. | Already optimal. Could optimize validation checks with early termination. | ✅ OK - Already optimal |
| Day12 | `JSONAbacus.java` | O(N) | O(N) | N = number of nodes in JSON tree. Recursive traversal visits each node once. | Already optimal. JSON parsing and traversal is necessary. | ✅ OK - Already optimal |
| Day13 | `HappinessCalculator.java` | O(n!) | O(n!) | Generates all permutations of people. n = number of people. Factorial complexity. | ✅ **Refactored**: Replaced permutation generation with recursive branch-and-bound approach. Already fixes first person to reduce from n! to (n-1)! permutations. More memory-efficient recursive traversal that builds arrangements incrementally. Calculates partial happiness during traversal for better performance. | ✅ OK - Optimized with recursive branch-and-bound, reduced memory usage |
| Day14 | `ReindeerRace.java` | O(R×T) | O(R×T) | R = reindeer, T = time steps. Part 2 uses efficient distance calculation per second. | ✅ **Refactored**: Extracted logic from `Day14.java` to `ReindeerRace.java`. Applied functional programming principles using Stream API (`IntStream`, `reduce`, `max`). Extracted `Reindeer` record to separate file. Uses immutable data structures and pure functions. Already optimized with O(1) distance calculation per reindeer per second. | ✅ OK - Optimized with efficient distance calculation and functional programming |
| Day15 | `CookieRecipe.java` | O(n^m) | O(n^m) | n = teaspoons (100), m = ingredients. Recursive combination generation. | ✅ **Refactored**: Extracted logic from `Day15.java` to `CookieRecipe.java`. Applied functional programming principles using immutable `List<Integer>` instead of mutable arrays. Extracted `Ingredient` and `RecipeScore` records to separate files. Uses Stream API and functional recursion. The recursive approach is already optimal for the problem size (4 ingredients, 100 teaspoons). | ✅ OK - Improved code quality with functional programming, algorithm already optimal for problem size |
| Day16 | `AuntSueDetector.java` | O(n) | O(n) | Linear scan through Aunt Sues. Each validation is O(1). | Already optimal. Early termination when match found. | ✅ OK - Already optimal |
| Day17 | `ContainerCombination.java` | O(2^n) with memoization | O(2^n) | Generates all combinations of containers with memoization. n = number of containers. | ✅ **Refactored**: Extracted logic from `Day17.java` to `ContainerCombination.java`. Applied functional programming principles using immutable lists and functional recursion. Uses Stream API for collection operations. Already optimized with memoization to avoid redundant calculations. | ✅ OK - Optimized with memoization and functional programming |
| Day18 | `LightGrid.java` | O(S×R×C) | O(S×R×C) | S = steps (100), R×C = grid size. Conway's Game of Life simulation. Uses `Grid` from third-party module and `GridDirections.countNeighbors()` for neighbor counting. | ✅ **Refactored**: Merged duplicate `simulateStep` and `simulateStepWithStuckCorners` methods into unified method with parameters. Cached corner positions in a Set for O(1) lookup instead of recalculating on every cell. Removed redundant corner position calculations. Improved code organization and reduced duplication. Maintains O(S×R×C) complexity with better constant factors. | ✅ OK - Optimized with code deduplication and corner caching |
| Day19 | `MoleculeReplacement.java` | O(M×L) | O(attempts×L) | M = replacements, L = molecule length. Part 2 uses optimized deterministic greedy with improved heuristics. | ✅ **Refactored**: Extracted logic from `Day19.java` to `MoleculeReplacement.java`. Applied functional programming principles using Stream API (`flatMap`, `groupingBy`, `mapping`, `toSet`). Extracted `ParsedInput` and `Replacement` records to separate files. Enhanced algorithm with better data structures. Improved sorting strategy: length-first then alphabetical for deterministic behavior. Optimized string operations using `substring` instead of regex `replaceFirst`. Uses mutable `ArrayList` for list rotation (necessary for algorithm). Reduced MAX_ATTEMPTS from 1000 to 100 while maintaining reliability. | ✅ OK - Optimized with improved greedy heuristics, data structures, and functional programming |
| Day20 | `InfiniteElvesAndInfiniteHouses.java` | O(H log H) | O(H log H) | H = house number to find. Uses sieve-based approach: iterate through elves and add presents to all houses they visit. | ✅ **Optimized**: Replaced sequential divisor calculation (O(H×√H)) with sieve-based approach (O(H log H)). For each elf e, add presents to houses e, 2e, 3e, ... Complexity improved from O(H×√H) to O(H log H) using harmonic series. Expected 2-5x performance improvement. Includes fallback to sequential method if estimate is too low. | ✅ OK - Optimized with sieve-based approach, improved complexity |
| Day21 | `RPGEquipment.java` | O(W×A×R²) | O(W×A×R²) | W = weapons (5), A = armor (6), R = rings (8). Nested loops over equipment. | ✅ **Refactored**: Extracted logic from `Day21.java` to `RPGEquipment.java`. Applied functional programming principles using nested streams for equipment combination generation. Extracted `Item`, `Character`, and `Equipment` records to separate files. Battle simulation converted to pure recursive function using immutable `Character` states. Uses Stream API for parsing and data transformations. Already optimal for small input space. | ✅ OK - Improved with functional programming, acceptable for small input space |
| Day22 | `WizardSimulator.java` | O(exp) with memoization | O(exp) with memoization | State space search with priority queue and memoization. Exponential state space with visited state tracking. | Already optimized with memoization to avoid revisiting states. | ✅ OK - Optimized with memoization |
| Day23 | `TuringComputer.java` | O(n) | O(n) | Linear execution of instructions. Simple virtual machine. | Already optimal. No improvements needed. | ✅ OK - Already optimal |
| Day24 | `PackageBalancer.java` | O(2^n) | O(2^n) | Generates all combinations to find target weight. n = number of packages. Uses recursive backtracking with branch-and-bound. | ✅ **Refactored**: Extracted logic from `Day24.java` to `PackageBalancer.java`. Applied functional programming principles using immutable lists and functional recursion with `append` helper. Implemented progressive size search with branch-and-bound optimization. Two-phase approach: first find minimum size, then find minimum quantum entanglement. Sorts packages in descending order for faster discovery. Multiple early pruning strategies. Memory-efficient: tracks only best QE instead of storing all combinations. ~18x performance improvement (3.6s → 0.2s). | ✅ OK - Optimized with branch-and-bound, progressive search, and functional programming |
| Day25 | `CodeGenerator.java` | O(log p) | N/A | p = position in sequence. Uses modular exponentiation for fast code generation. | ✅ **Refactored**: Extracted logic from `Day25.java` to `CodeGenerator.java`. Applied functional programming principles. Extracted `Position` record to separate file. Uses pure functions (`parsePosition`, `calculateSequencePosition`). Already optimized with fast modular exponentiation. | ✅ OK - Optimized with modular exponentiation and functional programming |
...
- **O(R×C) or O(S×R×C)**: Days 6, 18
- **O(n²) or O(V²)**: Days 7, 14 (Part 2)
...
1. ✅ **Day9 (RouteOptimizer)**: **COMPLETED** - Implemented recursive branch-and-bound approach, fixed first city to reduce from n! to (n-1)! permutations
2. ✅ **Day13 (HappinessCalculator)**: **COMPLETED** - Implemented recursive branch-and-bound approach, already fixes first person to reduce permutations
3. ✅ **Day15 (Day15)**: **COMPLETED** - Improved code quality and organization; recursive approach is optimal for problem size (4 ingredients, 100 teaspoons)
4. ✅ **Day17 (Day17)**: **ALREADY OPTIMIZED** - Already uses memoization to avoid redundant calculations
5. ✅ **Day24 (Day24)**: **COMPLETED** - Implemented progressive size search with branch-and-bound, ~18x performance improvement
6. ✅ **Day25 (Day25)**: **ALREADY OPTIMIZED** - Already uses modular exponentiation (O(log p)) with fast exponentiation algorithm
...
- Several days have exponential/factorial complexity but work well for small inputs and have been optimized:
  - **Day9**: Small number of cities (typically 7-8) - ✅ Optimized with recursive branch-and-bound, reduced memory usage
  - **Day13**: Small number of people (typically 8-9) - ✅ Optimized with recursive branch-and-bound, reduced memory usage
  - **Day15**: Constrained by 100 teaspoons - ✅ Code quality improved, algorithm optimal for problem size
  - **Day17**: Small number of containers - ✅ Already optimized with memoization
    - **Day20**: Large house numbers (~800K) - ✅ Optimized with sieve-based approach (O(H log H) vs O(H×√H), ~2-5x faster)
  - **Day24**: Small number of packages - ✅ Optimized with progressive search and branch-and-bound (~18x faster)
  - **Day25**: Uses modular exponentiation - ✅ Already optimized with O(log p) complexity
...
**Optimization:**
- ✅ **Implemented**: Recursive branch-and-bound approach
- ✅ **Implemented**: Fixed first city to reduce from n! to (n-1)! permutations
- ✅ **Implemented**: Memory-efficient recursive traversal instead of generating all permutations upfront
- Note: Held-Karp algorithm (O(2^n × n²)) could provide further optimization for very large inputs, but current approach is optimal for typical problem sizes
...
**Optimization:**
- ✅ **Implemented**: Recursive branch-and-bound approach
- ✅ **Implemented**: Already fixes first person to reduce from n! to (n-1)! permutations
- ✅ **Implemented**: Memory-efficient recursive traversal that builds arrangements incrementally
- ✅ **Implemented**: Calculates partial happiness during traversal for better performance
...
- Actual combinations: C(100+4-1, 4-1) = C(103, 3) ≈ 176,851

**Optimization:**
- ✅ **Implemented**: Extracted logic from `Day15.java` to `CookieRecipe.java`
- ✅ **Implemented**: Applied functional programming with immutable `List<Integer>` instead of mutable arrays
- ✅ **Implemented**: Extracted `Ingredient` and `RecipeScore` records to separate files
- ✅ **Implemented**: Uses Stream API and functional recursion
- ✅ **Implemented**: Improved code organization and readability
- ✅ **Implemented**: Better variable naming and method structure
...
**Optimization:**
- ✅ **Implemented**: Extracted logic from `Day17.java` to `ContainerCombination.java`
- ✅ **Implemented**: Applied functional programming with immutable lists and functional recursion
- ✅ **Already Implemented**: Dynamic programming with memoization (`countCombinationsMemoized`)
- ✅ **Already Implemented**: Early termination when target reached
- ✅ **Already Implemented**: Knapsack-style optimization approach
...
**Optimization:**
- ✅ **Implemented**: Extracted logic from `Day24.java` to `PackageBalancer.java`
- ✅ **Implemented**: Applied functional programming with immutable lists and functional recursion
- ✅ **Implemented**: Progressive size search with branch-and-bound
- ✅ **Implemented**: Two-phase approach (find min size, then min QE)
- ✅ **Implemented**: Sorting packages in descending order
- ✅ **Implemented**: Multiple early pruning strategies
- ✅ **Implemented**: Memory-efficient tracking (no storage of all combinations)
- Performance improvement: ~18x faster (3.6s → 0.2s)
...
**Optimization:**
- ✅ **Implemented**: Extracted logic from `Day25.java` to `CodeGenerator.java`
- ✅ **Implemented**: Extracted `Position` record to separate file
- ✅ **Implemented**: Applied functional programming with pure functions
- ✅ **Already Implemented**: Modular exponentiation with O(log p) complexity
- ✅ **Already Implemented**: Fast exponentiation algorithm (`modPow` method)
- ✅ **Already Implemented**: Formula: code = (20151125 × 252533^(position-1)) mod 33554393
...
**Previous Approach:**
- Sequential house-by-house checking: O(H × √H)
- For each house H, calculate divisors up to √H
- Total operations: ~700M for H=800K

**Optimization:**
- ✅ **Implemented**: Replaced sequential divisor calculation with sieve-based approach
- ✅ **Implemented**: Complexity improved from O(H × √H) to O(H log H)
- ✅ **Implemented**: For Part 1: each elf e visits houses e, 2e, 3e, ... up to H
- ✅ **Implemented**: For Part 2: each elf e visits houses e, 2e, ..., 50e (limited to 50 houses)
- ✅ **Implemented**: Includes fallback to sequential method if sieve estimate is too low
- ✅ **Implemented**: Better cache locality with sequential memory access pattern
...
- Classes renamed to remove "Solver" suffix (e.g., `ReindeerRaceSolver` → `ReindeerRace`)
...
- Pure functions for parsing and calculations
- Improved clarity with constants and better method structure
...
2. **Day6 (LightCounter)**:
   ...
   - Enhanced code maintainability while preserving O(n×A) complexity
...
4. **Day19 (MoleculeReplacement)**:
   ...
   - More efficient and maintainable solution while preserving O(attempts×L) complexity
...
8. **Day24 (PackageBalancer)**:
   ...
   - Significant performance improvement: ~18x faster (3.6s → 0.2s)
...
9. **Day18 (LightGrid)**:
   ...
   - Better constant factors while maintaining O(S×R×C) complexity
...
10. **Day20 (InfiniteElvesAndInfiniteHouses)**:
   ...
   - Complexity improved from O(H×√H) to O(H log H) using harmonic series
...
*Last updated: 2025-12-06 (Architectural refactoring: logic extraction, functional programming, and Day20 optimization)*
