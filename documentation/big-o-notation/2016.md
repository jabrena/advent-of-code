...
| Day5 | [`HowAboutANiceGameOfChess.java`](../../2016/src/main/java/info/jab/aoc2016/day5/HowAboutANiceGameOfChess.java) | O(k) | O(k) | 3.210s | k = number to find (MD5 hash). Brute force search. Part 2 test disabled (slow). | Already optimal for brute force. Could parallelize MD5 computation. | 🔴 HIGH - Part 2 disabled, slow execution (3.2s) |
...
| Day11 | [`RTGFacility.java`](../../2016/src/main/java/info/jab/aoc2016/day11/RTGFacility.java) | O(exp) | O(exp) | 1.196s | BFS state space search. Exponential state space. | Uses BFS with visited set. Could add better heuristics or A*. | 🟡 MEDIUM - Exponential but acceptable (1.2s) |
| Day12 | [`AssembunnyInterpreter.java`](../../2016/src/main/java/info/jab/aoc2016/day12/AssembunnyInterpreter.java) | O(n) | O(n) | 0s (disabled) | n = instructions. Linear execution. Test class disabled. | Already optimal. Simple interpreter. | 🟡 MEDIUM - Tests disabled |
...
| Day14 | [`OneTimePad.java`](../../2016/src/main/java/info/jab/aoc2016/day14/OneTimePad.java) | O(k×1000) | O(k×1000×2016) | 0s (disabled) | k = keys to find. Part 2 uses key stretching (2016 hashes). Test class disabled. | Uses hash caching. Already optimized. | 🟡 MEDIUM - Tests disabled |
...
**Complexity:** O(k×1000) / O(k×1000×2016)
- k = keys to find
...
1. **Day5 (HowAboutANiceGameOfChess) - Slowest execution (3.210s), Part 2 disabled**
...
3. **Day14 (OneTimePad) - Test class disabled**
   - **Action:** Investigate why tests are disabled
   - **Options:**
     - Re-enable tests if implementation is correct
...
5. **Day15 (TimingIsEverything) - Fast but could be optimized (0.019s)**
