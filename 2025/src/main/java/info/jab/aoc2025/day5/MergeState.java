 ...
 */
record MergeState(Range current, List<Range> merged, int index) {
    /**
     ...
     */
    boolean hasNext(final List<Range> sortedRanges) {
     ...
     */
    MergeState next(final List<Range> sortedRanges) {
        final Range next = sortedRanges.get(index + 1);
