 ...
 */
public class SoupLetter implements Solver<Integer> {
                ...
                return data.stream()
                                .mapToInt(characters -> IntStream.range(0, characters.size() - 3)
                                                .mapToObj(index -> "" + characters.get(index)
                                                                + characters.get(index + 1) + characters.get(index + 2)
                                                                + characters.get(index + 3))
        ...
        private int xmasCount() {
                return getHorizontalCount() + getVerticalCount() + getForwardDiagonalCount()
                                + getBackwardDiagonalCount();
        }

        private int xDashMasCount() {
                return IntStream.range(1, data.size() - 1)
                                .map(lineIndex -> IntStream.range(1, data.getFirst().size() - 1)
                                                .map(columnIndex -> data.get(lineIndex).get(columnIndex) == 'A'
                                                                && this.findWordInX(
                                                                                "" + data.get(lineIndex - 1)
                                                                                                .get(columnIndex - 1)
                                                                                                + 'A'
                                                                                                + data.get(lineIndex
                                                                                                                + 1)
